      ...
      m_lvfc { nullptr }
{
    QObject::connect(m_engine, &QQmlApplicationEngine::objectCreated, this, [this](QObject *object, const QUrl &url) {
        if (!object) {
            // QML failed to load (e.g. missing plugin); objectCreated fires with nullptr — dereferencing it segfaults.
            qWarning() << "FocusController: objectCreated fired with a null root object for" << url.toString();
            return;
        }
