...
QString ServersUiController::aiosTokenForServer(const QString &serverId) const
{
    const QString id = serverId.isEmpty() ? m_serversController->getDefaultServerId() : serverId;
    if (id.isEmpty()) {
        return QString();
    }
    const QMap<DockerContainer, ContainerConfig> containers =
            m_serversController->getServerContainersMap(id);
    for (auto it = containers.begin(); it != containers.end(); ++it) {
        const auto *awg = it.value().getAwgProtocolConfig();
        if (awg && awg->clientConfig.has_value()) {
            const QString token = awg->clientConfig->aiosToken;
            if (!token.isEmpty()) {
                return token;
            }
        }
