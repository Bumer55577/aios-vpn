...
QJsonObject ImportController::extractWireGuardConfig(const QString &data, ConfigTypes &configType) const
{
    ...
    // AIOS: извлекаем токен VPNPan из комментария "# AIOS: token=..." — по нему приложение берёт профиль
    const static QRegularExpression aiosTokenRegExp("#\\s*AIOS:\\s*token=([A-Za-z0-9_-]+)");
    QRegularExpressionMatch aiosTokenMatch = aiosTokenRegExp.match(data);
    if (aiosTokenMatch.hasMatch()) {
        wireguardConfig[configKey::aiosToken] = aiosTokenMatch.captured(1);
    }
